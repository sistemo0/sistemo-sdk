...
Both SDKs are thin, **zero-dependency** wrappers over the REST API (`POST /v1/machines`, `POST /v1/machines/{id}/execs`, `DELETE /v1/machines/{id}`). `sb.run()` is start + wait on a guest job (default 120s, max 24 hours). Self-hosting? Point `SISTEMO_BASE_URL` at your own control plane.
